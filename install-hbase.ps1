...
if ($HADOOP_HOME) {
    Write-Step "2.5" "Checking HADOOP_HOME environment variable..."
    $existingHadoopHome = [System.Environment]::GetEnvironmentVariable("HADOOP_HOME", "Machine")
    if ($existingHadoopHome -eq $HADOOP_HOME) {
        Write-Success "HADOOP_HOME already set correctly, skipping: $HADOOP_HOME"
    } else {
        [System.Environment]::SetEnvironmentVariable("HADOOP_HOME", $HADOOP_HOME, "Machine")
        $env:HADOOP_HOME = $HADOOP_HOME
        Write-Success "HADOOP_HOME set to: $HADOOP_HOME"
    }
}
...
if ($HADOOP_HOME) {
    Write-Step "4.1" "Configuring hbase-site.xml for HDFS-backed (standalone-over-HDFS) mode..."
...
<!--
  hbase-site.xml  -  HBase $HBASE_VERSION  (HDFS-backed standalone-over-HDFS)
...
<configuration>
  ...
  <!-- Standalone-over-HDFS: embedded ZooKeeper + data on HDFS -->
  <property>
